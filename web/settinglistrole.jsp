                            ...
                            <div class="col-12 mt-4">
                                <div class="table-responsive shadow rounded">
                                    <table class="table table-center bg-white mb-0">
                                        <thead>
                                            <tr>
                                                <th class="border-bottom p-3" style="min-width: 50px;">role Id</th>
                                        ...
                                        <tbody>
                                            <c:forEach var="role" items="${roleList}">
                                                <tr>
                                                    <td class="p-3">${role.roleId}</td>
                                                    <td class="p-3">${role.roleName}</td>
                                                    <td class="p-3">${role.roleCount}</td>


                                                    <td class="p-3">
                                                        <span class="badge ${role.status == 1 ? 'bg-soft-success' : 'bg-soft-warning'}">
                                                            ${role.status == 1 ? 'Kích hoạt' : 'Ngừng kích hoạt'}
                                                        </span>
                                                    </td>
                                                    <td>
                                                        <form action="settinglistblog" method="POST" style="display: inline;">
                                                            <input type="hidden" name="blogTopicId" value="${role.roleId}">
                                                            <input type="hidden" name="status" value="${role.status == 1 ? 0 : 1}">
                                                            <button type="submit" class="btn btn-icon btn-pills ${topic.status == 1 ? 'btn-soft-danger' : 'btn-soft-success'}">
                                                                <span class="dashicons ${role.status == 1 ? 'uil uil-times' : 'uil uil-check'}"></span>
                                                            </button>
                                                        </form>
                                                        <a href="settingblogdetail?blogTopicId=${role.roleId}" class="btn btn-icon btn-pills btn-soft-success"><i class="uil uil-pen"></i></a>
                                                    </td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
